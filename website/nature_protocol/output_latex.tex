...
\paragraph*{A.  Reference data preparation}
...
\subsubsection*{Data Preprocessing (Step 3)}

\paragraph*{A.  Genotype preprocessing}


\paragraph*{B.  Phenotype preprocessing}
...
\paragraph*{A.  QTL association testing}


\subsubsection*{Multivariate Mixture Models (Step 5)}

\subsubsection*{Multiomics Regression Models (Step 6)}

\paragraph*{A.  Fine-mapping and TWAS analysis}
...
\subsubsection*{Reference data preparation}
Timing: TBD

\paragraph*{1. Download the human genome}
...
\paragraph*{2. Download the gene annotation}
...
\paragraph*{3. Download the ERCC reference}
...
\paragraph*{5. Format and index the genome}
...
\paragraph*{6. Format the gene annotation}
...
\paragraph*{7. Combine gene and ERCC annotations}
...
\paragraph*{8. Build the STAR index}
...
\paragraph*{9. Build the RSEM index}
...
\paragraph*{10. Generate RefFlat annotation}
...
\paragraph*{11. Annotate a study VCF with dbSNP identifiers (optional)}
...
\paragraph*{12. Construct generalized TAD boundaries (optional)}
...
\paragraph*{13. Construct an LD sketch for RSS analysis (optional)}
...
\paragraph*{1. Assess FASTQ read quality}
...
\paragraph*{2. Trim sequencing adapters}
...
\paragraph*{3. Align reads with STAR and run Picard QC}
...
\paragraph*{4. Quantify gene-level expression with RNA-SeQC}
...
\paragraph*{5. Quantify transcript-level expression with RSEM}
...
\paragraph*{6. Perform cohort-level expression QC}
...
\paragraph*{7. Normalize the QC-passed expression matrices}
...
\paragraph*{1. Quantify intron usage with LeafCutter}
...
\paragraph*{2. Normalize LeafCutter ratios}

**What it does:** `leafcutter\_norm` filters introns and clusters, mean-imputes retained missing values, and quantile-normalizes the ratio matrix.
...
\paragraph*{3. Annotate LeafCutter phenotypes}

**What it does:** `annotate\_leafcutter\_isoforms` maps introns to genes and writes the coordinate-sorted phenotype matrix and phenotype-group file used by TensorQTL.
...
\subsubsection*{Genotype preprocessing}
Timing: ~3-5 min (on the toy dataset)

\paragraph*{1. Quality-control the input VCF}
...
\paragraph*{2. Convert the QC-passed VCF to PLINK and merge chromosomes}
...
\paragraph*{3. Apply PLINK-level quality control}
...
\paragraph*{4. Partition the QC-passed genotype data by chromosome}
...
\paragraph*{5. Match genotype and molecular-phenotype samples}
...
\paragraph*{6. Estimate kinship and separate related individuals}
...
\paragraph*{7. Prepare the unrelated, LD-pruned PCA subset}
...
\paragraph*{8. Estimate principal components in unrelated individuals}
...
\paragraph*{9. Extract the related samples at the PCA variants}
...
\paragraph*{10. Project related samples and detect PCA outliers}
...
\paragraph*{11. Remove projected PCA outliers}
...
\paragraph*{12. Recombine unrelated and projected related samples}
...
\subsubsection*{Phenotype preprocessing}
Timing: <12 min (on the toy dataset)

\paragraph*{1. Impute missing phenotype values}
...
\paragraph*{2. Add genomic coordinates to gene or protein phenotypes}
...
\paragraph*{3. Retrieve gene coordinates from Ensembl BioMart}
...
\paragraph*{4. Map LeafCutter clusters to genes}
...
\paragraph*{5. Annotate LeafCutter isoforms}
...
\paragraph*{6. Partition a BED phenotype by chromosome}
...
\paragraph*{7. Partition a GCT phenotype by chromosome}
...
\paragraph*{8. Partition a phenotype by predefined regions}
...
\paragraph*{9. Define TAD-based phenotype regions}
...
\paragraph*{10. Extract selected samples from a GCT matrix}
...
\paragraph*{11. Subset BAM files by genomic region}
...
\paragraph*{1. Merge observed covariates and genotype PCs}
...
\paragraph*{2. Infer PCA factors with Marchenko–Pastur selection}
...
\paragraph*{3. Infer PEER factors}
...
\paragraph*{4. Infer configurable PCA factors}
...
\paragraph*{5. Infer factors with bi-cross-validation}
...
\subsubsection*{QTL association testing}
Timing: TBD

\paragraph*{1. cis-QTL scan}
...
\paragraph*{2. trans-QTL scan}

**What it does:** Tests the selected traits against variants on chromosome 22, restricting traits to the identifiers listed in `data/combined\_AD\_genes.csv`.
...
\paragraph*{3. interaction-QTL scan}
...
\subsection*{5. Multivariate Mixture Models}


\subsection*{6. Multiomics Regression Models}


\subsubsection*{Fine-mapping and TWAS analysis}
Timing: TBD

\paragraph*{1. Univariate fine-mapping and TWAS}

**What it does:** `qtl\_dataset\_construct+susie\_twas` builds the regional dataset, fits SuSiE and saves fine-mapping results and cross-validated TWAS weights.
...
\paragraph*{2. Multivariate fine-mapping}
...
\paragraph*{3. Multigene multivariate fine-mapping}

**What it does:** `mnm\_genes` coordinates multivariate fine-mapping across genes while preserving phenotype identifiers and a common retained-sample set.
...
\paragraph*{4. Functional fine-mapping}
...
\paragraph*{5. Summary-statistic fine-mapping}
