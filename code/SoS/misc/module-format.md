...
Toy commands use repository-relative `tests/fixtures/...` paths when the required fixture is shipped. Intermediate products created by an earlier command use `output/...`. When the repository does not ship an input, use an angle-bracketed `<path/to/file>` placeholder; never invent an `input/...` path.
